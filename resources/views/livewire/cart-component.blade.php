                            ...
                            @if (Cart::count() > 0)
                                @foreach (Cart::content() as $item)
                                    <div class="rounded-lg md:w-2/3 xl:w-full">
                                        <div
                                            class="justify-between mb-6 rounded-lg bg-white p-6 shadow-md sm:flex sm:justify-start md:flex md:justify-start">
                                            <img src="{{ $item->model->image }}" alt="product-image"
                                                class="w-full rounded-lg sm:w-40" />
                                            <div class="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                                                <div class="mt-3 sm:mt-0">
                                                    <h2 class="text-lg font-bold text-gray-900 mr-4">
                                                        {{ $item->model->name_product }}</h2>
                                                    <!-- <p class="mt-1 text-xs text-gray-700">Empuk bet coy asli dah.</p> -->
                                                </div>
                                                <div
                                                    class="mt-2 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
                                                    <div class="flex items-center border-gray-100">
                                                        <form id="submits1" action="/dec-qty" method="GET"
                                                            class="">
                                                            @csrf
                                                            <input type="hidden" name="rowId"
                                                                value="{{ $item->rowId }}">
                                                            <button type="submit"
                                                                class="cursor-pointer rounded-l bg-gray-100 py-1 px-3.5 duration-100 hover:bg-primary-1 hover:text-blue-50">-</button>
                                                        </form>
                                                        ...
                                                        <span
                                                            class="h-8 w-8 border bg-white text-center text-xs outline-none p-2">{{ $item->qty }}</span>
                                                        <form id="submits2" action="/add-qty" method="GET"
                                                            class="">
                                                            @csrf
                                                            <input type="hidden" name="rowId"
                                                                value="{{ $item->rowId }}">
                    ...
                    @if (Cart::count() > 0)
                        <div class="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                            ...
                            <form action="/checkout" method="POST">
                                ...
                                <button type="submit"
                                    class="mt-6 w-full rounded-md bg-primary-1 py-1.5 font-medium text-white hover:bg-primary-2">Order</button>
                            </form>
                        </div>
                    @else
                        <div class="mt-10 mb-10 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3">
                            ...
                            <a href="{{ 'catalog' }}">
                                <button
                                    class="w-full rounded-md bg-primary-1 py-1.5 font-medium text-white hover:bg-primary-2">Go
                                    to Catalog</button>
                            </a>
                        </div>
                    @endif
